...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
   ...
   */
   ...
   for(unsigned int i = 0; i < successor_states_vector.size(); ++i){
       ...
       vector<Vehicle> trajectory = generate_trajectory(successor_states_vector.at(i), predictions);
   ...
   if(costs.size()> 0)
       ...
       return generate_trajectory(state, predictions);


  /**
   * TODO: Change return value here:
   */
  return generate_trajectory(successor_states_vector.at(min_cost), predictions);
}
